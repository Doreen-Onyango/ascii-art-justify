...
// 		return line + strings.Repeat(" ", width-len(line))
// 	}
// 	totalSpaces := width - len(line) + (len(words) - 1)
// 	spaceBetweenWords := totalSpaces / (len(words) - 1)
// 	extraSpaces := totalSpaces % (len(words))
// 	s := strings.Repeat(" ", spaceBetweenWords) + words
// 	for i := 0; i < extraSpaces; i++ {
// 		words += " "
// 	}
// 	// }
// 	return s //strings.Join(words, strings.Repeat(" ", spaceBetweenWords))
// }
